...
// grabbing length for the number of iterations within the for loop
    var pizzaContainerArrayLength = document.getElementsByClassName("randomPizzaContainer").length;
// isolating the elements into a target array
    var pizzaContainerArray = document.getElementsByClassName("randomPizzaContainer");
// dx will always be the same so no need to check offset width every single time.
// new_width is also the same. Checks width of first element.
    var dx = determineDx(pizzaContainerArray[0], size);
    var new_width = (pizzaContainerArray[0].offsetWidth + dx) + "px";
// changing the elements width
    for (i = 0; i < pizzaContainerArrayLength; i++) {
      pizzaContainerArray[i].style.width = new_width;
    }
  }
  changePizzaSizes(size);
  // huge improvement from 100ms+ to ~1ms^^^^^^^^^
...
// this loop adds ~17ms to load time...not sure how to fix without just adding the 100 divs to the HTML.
...
function updatePositions() {
  ...
  // It has to call on the document and then do math everytime the scroll event happens. Better to isolate that number.
  // thinking I can turn this into two separate loops???
  var minimizingTheMath = (document.body.scrollTop / 1250);
  for (var i = 0; i < itemsLength; i++) {
    var phase = Math.sin((minimizingTheMath) + (i % 5));
...
// created a row variable to calculate how many rows there are in the viewport
// multiplied rows and columns to create minimum number of moving pizzas needed to cover the viewport.
// however, this fails if you resize to a larger window after loading. Great for mobile. Not so great for desktops.
// moved the call to the document.body from the loop.
